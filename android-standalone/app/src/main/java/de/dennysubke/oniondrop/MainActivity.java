...
public final class MainActivity extends Activity {
    ...
    private void home(){
        ...
        center(hero,s.ready()?gs(R.string.phone_reachable):s.phase.equals("stopping")?gs(R.string.tor_stopping_message):s.active()?gs(R.string.tor_connecting_message):gs(R.string.hero_idle_subtitle),13,MUTED,false);
    ...
    private void navigation(){LinearLayout nav=row();nav.setPadding(dp(14),dp(8),dp(14),dp(8));nav.setBaselineAligned(false);String[] names={gs(R.string.nav_overview),gs(R.string.nav_send),gs(R.string.nav_receive)},icons={"home","send","receive"};for(int i=0;i<3;i++){final int dest=i;LinearLayout item=col();item.setGravity(Gravity.CENTER);item.setPadding(dp(4),dp(10),dp(4),dp(10));if(tab==i)item.setBackground(background(0xff2d1d3d,17));Icon icon=new Icon(icons[i],tab==i?PURPLE:MUTED);item.addView(icon,new LinearLayout.LayoutParams(dp(23),dp(23)));gap(item,5);TextView caption=text(names[i],11,tab==i?PURPLE:MUTED,tab==i);caption.setGravity(Gravity.CENTER);caption.setMaxLines(2);caption.setIncludeFontPadding(false);item.addView(caption,new LinearLayout.LayoutParams(-1,-2));item.setMinimumHeight(dp(72));item.setSelected(tab==i);item.setClickable(true);item.setFocusable(true);item.setContentDescription(names[i]);item.setOnClickListener(v->{if(!busy){tab=dest;render(true);}});LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,-2,1);p.setMargins(dp(3),0,dp(3),0);nav.addView(item,p);}root.addView(nav);}
    ...
    private void language(){AppLanguage.choose(this);}
    private void about(){new AlertDialog.Builder(this).setTitle(gs(R.string.about_title)).setMessage(gs(R.string.about_message,BuildConfig.VERSION_NAME)).setNeutralButton(R.string.language,(d,w)->language()).setPositiveButton(gs(R.string.understood),null).show();}
