...
public final class DropService extends Service {
    ...
    private final Runnable expire=()->finishSession(AppLanguage.wrap(DropService.this).getString(R.string.session_expired),false);
    @Override public void onCreate(){super.onCreate();NotificationChannel c=new NotificationChannel(CHANNEL,AppLanguage.wrap(DropService.this).getString(R.string.channel_name),NotificationManager.IMPORTANCE_LOW);c.setDescription(AppLanguage.wrap(DropService.this).getString(R.string.channel_desc));getSystemService(NotificationManager.class).createNotificationChannel(c);}
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        if(intent==null){stopSelf();return START_NOT_STICKY;}
        if(STOP.equals(intent.getAction())){finishSession(AppLanguage.wrap(DropService.this).getString(R.string.session_stopped_saved),false);return START_NOT_STICKY;}
        ...
        try{
            foreground(AppLanguage.wrap(DropService.this).getString(R.string.tor_connecting_message));
            if(server==null){
                ...
                tor=new TorRunner(new TorRunner.Listener(){
                    @Override public void progress(int percent){handler.post(()->{if(!stopping&&(!state.ready()||percent<100))publish("starting","",percent==100?AppLanguage.wrap(DropService.this).getString(R.string.onion_publishing):AppLanguage.wrap(DropService.this).getString(R.string.tor_connecting_message),percent);});}
                    @Override public void published(String host){handler.post(()->{if(!stopping){server.setOnion(host);publish("ready",host,AppLanguage.wrap(DropService.this).getString(R.string.phone_reachable),100);}});}
            ...
            if(START_SEND.equals(action)){
                if(((DropApp)getApplication()).store().shared().isEmpty())throw new IOException(AppLanguage.wrap(DropService.this).getString(R.string.choose_first));server.sending.set(true);
            }
            if(START_RECEIVE.equals(action))server.receiving.set(true);
            publish(state.ready()?"ready":"starting",state.ready()?state.host:"",state.ready()?AppLanguage.wrap(DropService.this).getString(R.string.phone_reachable):AppLanguage.wrap(DropService.this).getString(R.string.tor_connecting_message),state.ready()?100:state.progress);
        }catch(Exception ex){finishSession(ex.getMessage()==null?AppLanguage.wrap(DropService.this).getString(R.string.share_start_failed):ex.getMessage(),true);}
    ...
    private Notification notification(String text){
        ...
        return new Notification.Builder(this,CHANNEL).setSmallIcon(R.drawable.ic_notification).setContentTitle(AppLanguage.wrap(DropService.this).getString(R.string.notification_title)).setContentText(text).setContentIntent(open).setOngoing(true).setCategory(Notification.CATEGORY_SERVICE).setOnlyAlertOnce(true).addAction(new Notification.Action.Builder(null,AppLanguage.wrap(DropService.this).getString(R.string.stop),stop).build()).build();
    }
    ...
    private void finishSession(String message,boolean error){
        if(stopping)return;stopping=true;state=new State("stopping","",AppLanguage.wrap(DropService.this).getString(R.string.tor_stopping_message),"","",0,false,false,0);handler.removeCallbacks(expire);
    ...
    @Override public void onTimeout(int startId,int fgsType){finishSession(AppLanguage.wrap(DropService.this).getString(R.string.background_ended),false);}
    @Override public void onDestroy(){finishSession(AppLanguage.wrap(DropService.this).getString(R.string.share_ended),false);handler.removeCallbacksAndMessages(null);engine.shutdown();super.onDestroy();}
